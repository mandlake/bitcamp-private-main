...
import { getUser, logout } from "@/app/api/post/route";
import { useRouter } from "next/navigation";
import { useEffect, useState } from "react";

const UserPageMini = (props: { data: string }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await getUser(props.data);
        setUser(res);
      } catch (error) {
        console.log(error);
      } finally {
        ...
        <>
          <div className="flex flex-row gap-5">
            ...
            <div className="flex flex-col">
              <p className="name text-[14px]">{user?.name}님 안녕하세요.</p>
              <p className="email text-[10px]">{user?.email}</p>
              <div className="w-[100px] h-[25px] bg-[var(--color-main)] flex justify-center">
                <input
                  type="submit"
                  value="logout"
